...
def download_and_analyze_attachments(issue_key):
    ...
    try:
        ...
        for att in attachments:
            ...
            # Download file
            try:
                file_response = requests.get(download_url, auth=get_jira_auth(), timeout=60)
                ...
                if 'text/html' in content_type:
                    print(f"Got HTML instead of file for {filename} -可能是認證問題")
                    continue
        ...
        if archive_path.endswith('.zip'):
            try:
                with zipfile.ZipFile(archive_path, 'r') as z:
                    ...
                    txt_files = [os.path.join(extract_to, n) for n in z.namelist() if n.endswith('.txt')]
            ...
            try:
                with tarfile.open(archive_path, 'r:*') as t:
                    ...
                    txt_files = [os.path.join(extract_to, m.name) for m in members if m.name.endswith('.txt')]
